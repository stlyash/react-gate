...
const Exam = (props) => {
  ...
  const [qimage, setqimage] = useState(
    require("./assets/2022/cs/cs_question_2022--" +
      (qdata?.[qindex]?.ques || "Unknown").toString() +
      ".png")
  );

  const [gaqstatus, setGaqstatus] = useState([2, 1, 1, 1, 1, 1, 1, 1, 1, 1]);
  const [csqstatus, setCsqstatus] = useState([
  ...
  useEffect(() => {
    if (qindex > 9) {
      const nextcsq = csqstatus.map((item, ind) => {
        if (ind === qindex - 10 && item === 1) {
          return 2;
        } else {
      ...
      setCsqstatus(nextcsq);
    }
  }, [qindex]);
  useEffect(() => {
    const nextGaq = gaqstatus.map((item, ind) => {
      if (ind === qindex && item === 1) {
        return 2;
      } else {
        // Return a new circle 50px below
        return item;
  ...
  function next() {
    if (qindex < 64) {
      setqimage(
        require("./assets/2022/cs/cs_question_2022--" +
          (qdata?.[qindex]?.ques + 1 || "Unknown").toString() +
          ".png")
      );
      setqindex(qindex + 1);
    }
  ...
  function gocs() {
    if (qindex <= 9) {
      ...
      setqimage(
        require("./assets/2022/cs/cs_question_2022--" +
          (qdata?.[10]?.ques || "Unknown").toString() +
          ".png")
      );
    }
  }
        ...
        {gaqstatus.map((i, ind) => (
          <div className="containertry listed" key={`status${ind}`}>
            <div className="imgstat" onClick={() => gotoq(ind)}>
              <img
                className="imgtry imgstat"
                src={require(`./assets/status/st${i}.jpg`)}
                alt={`status${i}`}
              />
              <div
                className="centeredtry censtat"
                style={i === 1 ? { color: "black" } : { color: "white" }}
              >
                {ind + 1}
              </div>
            </div>
  ...
  return (
    <div className="p-0 m-0">
      ...
      <div className="row sub-banner">
        <div className="col-6 subanner-heading">
          CS 2 Computer Science Information Technology
        </div>
        <div className="col-6 text-end">
          <div
            style={{ display: "inline" }}
            className="subanner-question"
          ...
          <div
            style={{ display: "inline" }}
            className="subanner-instruction"
            data-bs-toggle="modal"
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              <iframe
                ...
                src="https://drive.google.com/file/d/17nmrLNezouGzFGNr3ddjCmHU_1AgfXan/preview"
                width="100%"
                height="480rem"
                allow="autoplay"
              ></iframe>
      ...
      <div className="exam-body row p-0 m-0">
        <div className="exam-main col-9">
          ...
          <div className="exam-time row">
            ...
            <div
              className="indiv timl secsec col-6 text-end"
              style={{ paddingRight: "0.65cm" }}
            >
          ...
          <div className="exam-section row p-0 m-0">
            <div className="col-10">
              ...
              <button
                onClick={goga}
                style={{ backgroundColor: gacolor, color: cscolor }}
                className="gogab"
              >
                General Aptitude{" "}
                <img className="calc-info" src={infoIcon} alt="info icon" />
              ...
              <button
                onClick={gocs}
                style={{ backgroundColor: cscolor, color: gacolor }}
                className="gocsb"
              >
                CS Computer Science...{" "}
                <img className="calc-info" src={infoIcon} alt="info icon" />
          ...
          <div className="exam-type row pt-1">
            <div className="col-6 qtype">
              <div className="indiv">
                Question Type:{" "}
                {(qdata?.[qindex]?.type || "Unknown").toUpperCase()}
              </div>
            </div>
            <div className="col-6 text-end qmarks">
              <div className="indiv">
                Marks for Correct Answer:{" "}
                <span style={{ color: "#10de43", fontSize: "0.9rem" }}>
                  {qdata?.[qindex]?.mar || "Unknown"}
                </span>{" "}
              </div>
              <div className="indiv" style={{ paddingRight: "0.4cm" }}>
                | Negative Marks:{" "}
                <span style={{ color: "red", fontSize: "0.9rem" }}>
                  {(qdata?.[qindex]?.type || "Unknown")[1] === "c"
                    ? (qdata?.[qindex]?.mar || 0).toString() + "/3 "
                    : 0}
                </span>
              </div>
            </div>
          ...
          <div className="exam-question">
            <div className="indiv quesno">
              Question No. {qdata?.[qindex]?.ques || "Unknown"}
            </div>
          </div>
          ...
          <div className="textncontrol">
            ...
            <div className="exam-control row">
              <div className="col-6">
                <button onClick={next} className="ctlbtn mfr">
                  Mark for Review & Next
                </button>
                <button className="ctlbtn cr">Clear Response</button>
              </div>
              <div className="col-6 text-end">
                <button className="ctlbtn pr" onClick={previous}>
                  Previous
                </button>
                <button className="ctlbtn sv" onClick={next}>
                  Save & Next
                </button>
              </div>
            </div>
        ...
        <div className="exam-right col-3 m-0 p-0">
          ...
          <div className="status-panel">
                  ...
                  <div className="grid-cent">
                    <div className="containertry">
                      <img
                        ...
                        style={{ width: "100%" }}
                      />
                      <div className="centeredtry" style={{ color: "white" }}>
                        35
                      </div>
                    </div>
                    <div className="statex" style={{ color: "black" }}>
                      Answered
                    </div>
                  </div>
                  <div className="grid-cent">
                  ...
                  <div className="grid-cent">
                    <div className="containertry">
                      <img
                        ...
                        style={{ width: "100%" }}
                      />
                      <div className="centeredtry" style={{ color: "black" }}>
                        36
                      </div>
                    </div>
            ...
            <div className="status-section">General Aptitude</div>
            <div className="row">{gaQuestionStates()}</div>

            <div className="status-section">Computer Science</div>

            <div className="row">{csQuestionStates()}</div>
          </div>
